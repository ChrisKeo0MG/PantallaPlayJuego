...
var scoreNuevo = NomPlayer.score;
var powup;
...
function create ()
{
    ...
    platforms.create(500, 1985, 'ground').setScale(5,6).refreshBody();
    
    platform_Mov = this.physics.add.image(2850, 1299, 'ground').setScale(3.15, 6);
    
    // Asegurarse de que la plataforma tiene un cuerpo físico
    platform_Mov.body.allowGravity = false; // No queremos que caiga con la gravedad

    // Establecer el movimiento inicial de la plataforma
    platform_Mov.setVelocityX(100); // Mover la plataforma hacia la derecha

    // Hacer que la plataforma se detenga si colisiona con los límites del mundo
    platform_Mov.setCollideWorldBounds(true); 
    platform_Mov.setImmovable(true);
...
function update ()
{
    if (platform_Mov.body.blocked.left || platform_Mov.body.blocked.right) {
        cambiarDireccion();
    }
    console.log("Jugador en: "+player.y);
    if (player.y >= 1920) {  // Aquí 2094 es el límite inferior del mapa
        playerDie.call(this);
    }
...
function collectStar (player, star)
{
    ...
    stars.children.iterate(function (child) {
        if (child.y >= 1920) { // Si la estrella ha caído fuera de la pantalla
            child.enableBody(true, child.x, 0, true, true); // Regenerar la estrella en la parte superior
        }
    });
}
...
function cambiarDireccion() {
    if (platform_Mov.body.velocity.x > 0) {
        platform_Mov.setVelocityX(-300); // Cambiar la dirección a izquierda
    } else {
        platform_Mov.setVelocityX(300); // Cambiar la dirección a derecha
    }
}
...
function playerDie() {
    if (!gameOver) {
        ...
        // Aplicar un retroceso al jugador
        player.setVelocityY(-500);  // Hacer que el jugador "salte" un poco antes de morir
        player.setTint(0xff0000);  // Tintar al jugador de rojo

        // Pausar la física y las animaciones después de un breve retraso
